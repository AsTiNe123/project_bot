...
async def start_scheduler(tt, state: FSMContext):
    if not tt:
        ...
        scheduler.add_job(send_words, 
                          trigger = 'cron', 
                          hour=f"{morning_time.hour}-{night_time.hour}", 
                          minute=f"0-59/3", 
                          args= [state])
...
async def send_words(state: FSMContext):
    ...
    asyncio.create_task(skipping_task(msg, 120, state))
    await state.update_data({'state_of_task': "1"})
...
async def morning_time(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await rules(message)    
        await message.answer(text = "Всё, ты закончил регистрацию", reply_markup=answer)
        ...
        await message.answer(text = f"Ты не правильно ввёл время(<i><b>ПРИМЕР</b></i> 7:00)", parse_mode = "HTML")

@dp.message_handler(state = 'q4')
async def answer(message: types.Message, state: FSMContext):
    answer = message.text.split('\n')
    asyncio.create_task(delete_message(message, 120))
    while len(answer) < 5:
        answer.append('---')
    data = await state.get_data()
    ...
    asyncio.create_task(delete_message(ms, 120))
    await state.update_data({'state__of_task': '0'})
    await state.update_data({'score': score})
    await state.set_state("q3") 
...
async def ketboard(message: types.Message, state: FSMContext):
    if message.text == "Инструкция":
        ...
        await score(message, state)
    elif message.text == "Слова сегодня":
        data = await state.get_data()
        list_of_words = data["today_words"]
        list_of_words2 = "\n".join(list_of_words)
        await bot.send_message(data["chat_id"], "сегодня мы учим слова:\n"+list_of_words2)
    elif message.text == "Мой сегодняшний результат":
        data = await state.get_data()
        today_result: dict = data["today_result"]
        to_send = "Твои результаты сегодня:\n"
        for i in today_result.items():
            to_send += f"{i[0]}: {i[1]}\n"
